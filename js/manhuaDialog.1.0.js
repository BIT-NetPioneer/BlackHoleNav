...
$(function() {
    $.fn.manhuaDialog = function(options) {
        ...
        $("body").prepend("<div class='floatBoxBg' id='fb"+options.title+"'></div><div class='floatBox' id='"+options.title+"'><div class='dialog-title' id='t"+options.title+"'><h4></h4><span class='closeDialog' id='c"+options.title+"'>X</span></div><div class='content'></div></div>");	
        var $this = $(this);								//当然响应事件对象
        var $blank = $("#fb"+options.title);						//遮罩层对象
        var $title = $("#"+options.title+" .dialog-title h4");				//弹出层标题对象
        ...
        $this.live(options.Event,function(e){				
            ...
            switch(options.type){
                ...
                case "iframe":								//当类型是Iframe的时候
                    //$content.html("<iframe src=\""+options.content+"\" width=\"100%\" height=\""+(parseInt(options.height)-30)+"px"+"\" scrolling=\"auto\" frameborder=\"0\" marginheight=\"0\" marginwidth=\"0\" style=\"width="+ (parseInt(options.height)-30) +"\"></iframe>");
                    $content.html("<iframe src=\""+options.content+"\" style=\"height: "+ (parseInt(options.height)-32) +"px; width:"+ (parseInt(options.width)) +"px;\"></iframe>");
                    break;
                default:									//默认情况下的时候
                    $content.html(options.content);
                    break;
